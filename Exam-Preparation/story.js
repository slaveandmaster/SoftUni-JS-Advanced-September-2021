class Story {
  ...
  dislike(username) {
    ...
    this._likes = this._likes.filter((u) => username != u);
  ...
  comment(username, content, id) {
    ...
    let checkCommentId = this._comments.find((c) => c.Id == id);
    if (checkCommentId) {
      let replyID = Number(
        checkCommentId.Id + "." + checkCommentId.Replies.length + 1
      );
      let reply = {
        Id: replyID,
        Username: username,
      ...
      checkCommentId.Replies.push(reply);
  ...
  _sortCriteria(commentsOrReplies, criteria) {
    ...
    if (criteria === "asc") {
      sortedCommentsOrReplies = copyCommentsOrReplies.sort(
        (a, b) => a.Id - b.Id
      );
      ...
      sortedCommentsOrReplies = copyCommentsOrReplies.sort((a, b) =>
